...
def get_pokemon_details(selected_pokemon, theme):
    ...
    if pokemon_data:
        ...
        st.markdown(f"<p style='text-align: center; font-size: 0.5vw;'>Sorry we cannot find the image of {(pokemon_name)} 😔 </p>",
                unsafe_allow_html=True)
        st.markdown(
...
def display_tab():
    ...
    st.markdown("<div style='text-align: center; font-size: 2.2vw; font-weight: bold;'>Unleash the Poke-challenger within! 😈</div>",
                unsafe_allow_html=True)
    st.markdown(
        "<div style='text-align: center; font-size: 0.7vw;'>It's time to clash, compete, and catch 'em in the ultimate battle showdown! Let the Pokémon brawl begin!</div>",
        unsafe_allow_html=True)
    ...
    bg_space1, bg_1_1, bg_space2, bg_1_2, bg_space3, bg_1_3, bg_space4 = st.columns(
        (0.001, 0.4, 0.01, 0.2, 0.01, 0.4, 0.05)
    )
    ...
    with bg_1_1:
        ...
        selected_generation_1 = bgr_column1.selectbox("Generation", df["generation"].unique(),
                                                    key="generation_1")

        # Filter by Type based on selected Generation
        filtered_df_by_generation = df[df["generation"] == selected_generation_1]
        default_status = 'Normal'
        selected_status_1 = bgr_column2.selectbox("Select Status", filtered_df_by_generation["status"].unique(),
                                                index=filtered_df_by_generation["status"].unique().tolist().index(
                                                    default_status),
                                                key="status_1")

        # Filter by Type based on selected Generation
        filtered_df_by_generation = df[df["status"] == selected_status_1]
        default_type_2 = 'Electric'
        unique_types = filtered_df_by_generation["type_1"].unique().tolist()

        if default_type_2 in unique_types:
            default_type_index = unique_types.index(default_type_2)
        else:
            default_type_index = 0

        selected_type_1 = bgr_column3.selectbox("Select a Type", unique_types, index=default_type_index, key="type_1")


        # Filter by Name based on selected Generation and Type
        filtered_df_by_type = filtered_df_by_generation[filtered_df_by_generation["type_1"] == selected_type_1]

        selected_pokemon_1 = bgr_column4.selectbox("Select a Pokemon", filtered_df_by_type["name"], key="pokemon_1")

        if selected_type_1 in ["Fire", "Poison", "Electric", "Fighting", "Dragon"]:
            theme_1 = "#FFA78C"
        elif selected_type_1 in ["Water", "Dragon", "Steel"]:
            theme_1 = "#D4F1F9"
        elif selected_type_1 in ["Grass", "Ice", "Fairy", "Ghost"]:
            theme_1 = "#E7F4D3"
        elif selected_type_1 in ["Bug", "Normal", "Dark", "Ground", "Psychic", "Rock"]:
            theme_1 = "#F1D5AA"

        contender1 = selected_pokemon_1

        get_pokemon_details(contender1, theme_1)
    ...
    with bg_1_3:
        ...
        # Filter by Generation
        selected_generation_2 = bgr_column1.selectbox("Generation", df["generation"].unique(),
                                                    key="generation_2")

        # Filter by Type based on selected Generation
        filtered_df_by_generation = df[df["generation"] == selected_generation_2]
        selected_status_2 = bgr_column2.selectbox("Select Status", filtered_df_by_generation["status"].unique(),
                                                key="status_select_2")

        # Filter by Type based on selected Generation
        filtered_df_by_generation = df[df["status"] == selected_status_2]
        selected_type_2 = bgr_column3.selectbox("Select a Type", filtered_df_by_generation["type_1"].unique(),
                                              key="type_select_2")

        # Filter by Name based on selected Generation and Type
        filtered_df_by_type = filtered_df_by_generation[filtered_df_by_generation["type_1"] == selected_type_2]
        selected_pokemon_2 = bgr_column4.selectbox("Select a Pokemon", filtered_df_by_type["name"],
                                                 key="pokemon_select_2")
        if selected_type_2 in ["Fire", "Poison", "Electric", "Fighting", "Dragon"]:
            theme_2 = "#FFA78C"
        elif selected_type_2 in ["Water", "Dragon", "Steel"]:
            theme_2 = "#D4F1F9"
        elif selected_type_2 in ["Grass", "Ice", "Fairy", "Ghost"]:
            theme_2 = "#E7F4D3"
        elif selected_type_2 in ["Bug", "Normal", "Dark", "Ground", "Psychic", "Rock"]:
            theme_2 = "#F1D5AA"

        contender2 = selected_pokemon_2

        get_pokemon_details(contender2, theme_2)
